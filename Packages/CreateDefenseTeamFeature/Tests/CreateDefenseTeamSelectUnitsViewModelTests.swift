...
@testable import CreateDefenseTeamFeature
import Entity
import Foundation
import Mocks
import Testing
...
struct CreateDefenseTeamSelectUnitsViewModelTests {
    ...
    let sut: CreateDefenseTeamSelectUnitsViewModelType
    ...
    func saveTeamSuccess() async {
        ...
        // Then
        #expect(result == true)
        #expect(mockClient.createDefenseTeamMembersCallsCount == 1)
        #expect(mockClient.createDefenseTeamMembersReceivedMembers?.count == 1)
    }
    ...
    func saveTeamEmpty() async {
        ...
        // Then
        #expect(result == false)
        #expect(mockClient.createDefenseTeamMembersCallsCount == 0)
    }
    ...
    func saveTeamError() async {
        ...
        await sut.inputs.fetchUnits()
        sut.inputs.toggleSelection(unit)
        mockClient.createDefenseTeamMembersThrowableError = NSError(domain: "test", code: 0)
